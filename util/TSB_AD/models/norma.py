...
class NORMA():
    ...
    ## added 'normalize' by jj
    def __init__(self, pattern_length, nm_size, clustering='hierarchical',percentage_sel=0.4,overlapping_factor=1, sampling_division=10,number_of_cluster=10, normalize=True):
    ...
    def run(self, ts):
        ...
        if self.clustering == "hierarchical":
            ...
            # print('linkage:', dendogram)
            # dn = hac.dendrogram(dendogram)
            # print('dendo:', dn)
            # print('list cluster:', listcluster)
        ...
        join_n = compute_score(ts, self.pattern_length, nms, scores_nms, self.normalize)
...
def compute_score(ts, pattern_length, nms, scores_nms, normalize):
    ...
    all_join = []
    for index_name in range(len(nms)):            
        join = stumpy.stump(ts,pattern_length,nms[index_name],ignore_trivial = False, normalize=normalize)[:,0]
	    #join,_ = mp.join(nm_name + '/' + str(index_name),ts_name,len(nms[index_name]),len(ts), self.pattern_length)
        join = np.array(join)
	    #join = (join - min(join))/(max(join) - min(join))
        all_join.append(join)
...
def choose_normalmodel(listcluster,recurrent_sequence, sequence_rec):
    ...
    for k in range(len(set(listcluster))):  # 서로 다른 클러스터의 수만큼
        mean = pd.DataFrame()
        count = 0
        index_seq = []
        for i in range(len(listcluster)):   # 모든 갯수만큼
            if listcluster[i] == k+1 :      # 클러스터 숫자가 같으면 (1부터)
                index_seq.append(sequence_rec[i])
                mean_t = np.mean(recurrent_sequence[str(i)].values)     # seq.의 평균값 for norm
                # std_t = np.std(recurrent_sequence[str(i)].values)

                data_to_join = [(float(j) - mean_t)/1 for j in recurrent_sequence[str(i)].values] # normalized
                mean[str(count)] = data_to_join
                count += 1

        cluster_mean.append(mean.mean(axis=1).values)   # 같은 클러스터의 평균
        score_weight.append(count)  # 같은 클러스터에 속한 subsequence 갯수

        i_seq_m = [ (i_s[1] + i_s[0])/2 for i_s in index_seq]   # index의 중간값
        all_index_p.append(i_seq_m)
        score_time.append(np.mean(i_seq_m))     # 중간값들의 평균 (얼마나 고루 퍼져있나)
        min_max_index.append([np.min(i_seq_m), np.max(i_seq_m)]) # 가장 작은 index와 가장 큰 index (역시 얼마나 고루 퍼져있나)
